...
package de.miraculixx.bmdm.utils

import de.miraculixx.bmdm.consoleAudience
import de.miraculixx.bmdm.prefix
import org.yaml.snakeyaml.DumperOptions
import org.yaml.snakeyaml.Yaml
import java.io.File
import java.io.InputStream

class YMLConfig(stream: InputStream?, val name: String, private val destination: File) {
    ...
    inline fun <reified T> get(key: String): T? {
        ...
        key.split('.').forEach {
            if (route is Map<*, *>) {
                ...
                consoleAudience.sendMessage(prefix + cmp("Failed to resolve config key '$key' because '$it' is no section or null ('$name.yml')"))
                return null
            }
        }
        return if (route is T) route as T else {
            consoleAudience.sendMessage(prefix + cmp("Failed to cast config value for key '$key' in config '$name.yml'"))
            null
        }
    }
